    ...
    try {
        ...
        players.forEach((p, idx) => {
            try {
                ...
                const progress = p?.elements?.progress?.container || p?.elements?.progress || null;
                    ...
                    if (container) {
                        container.addEventListener('keydown', (e) => {
                            ...
                            if (key === 'ArrowLeft' || key === 'Left') {
                                ...
                                try { p.currentTime = Math.max(0, (p.currentTime || 0) - 5); } catch(_){}
                                return false;
                            }
                        }, true);
                    }
...
function attachYoutubeQualityMenu(player, hostEl){
    try {
        ...
        const aspect = (hostEl && hostEl.closest && hostEl.closest('.aspect-video')) || null;
        const container = aspect || player?.elements?.container || hostEl?.parentElement || document.body;
        ...
        function getQualitiesSafe(){ try { return yt.getAvailableQualityLevels() || []; } catch(_) { return []; } }
        function getCurQ(){ try { return yt.getPlaybackQuality?.() || 'default'; } catch(_) { return 'default'; } }
        function resolveAvailable(desired){
            try {
                ...
                getQualitiesSafe().forEach((q) => { if (q) avail[q] = true; });
                const idx = order.indexOf(desired);
                for (let i = (idx < 0 ? 0 : idx); i < order.length; i++) {
                    const q = order[i]; if (avail[q]) return q;
                }
                return 'default';
            ...
            try {
                ...
                if (target === 'default') {
                    ...
                    if (typeof yt.setPlaybackQualityRange === 'function') {
                        try { yt.setPlaybackQualityRange(target, target); } catch(_){ }
                    }
                    yt.setPlaybackQuality(target);
                    [200, 500, 1000].forEach((ms) => setTimeout(() => { try { if (yt.getPlaybackQuality?.() !== target) yt.setPlaybackQuality(target); } catch(_){} }, ms));
                }
            } catch(_){}
        }
        function rebuildMenu(){
            ...
            levels.forEach((q) => { if (!q) return; const k = (q.toLowerCase() === 'auto') ? 'default' : q; if (!seen[k]) { seen[k] = true; normalized.push(k); } });
            const items = ['default'].concat(normalized.filter((q) => q !== 'default'));
            const cur = (() => { const c = getCurQ(); return (c === 'auto') ? 'default' : c; })();
            menu.innerHTML = '';
            items.forEach((q) => {
                ...
                opt.style.background = (q === cur) ? 'rgba(255,255,255,0.15)' : 'transparent';
                opt.addEventListener('click', (e) => { e.stopPropagation(); setQ(q); hideMenu(); });
                menu.appendChild(opt);
            });
        }
        function toggleMenu(){ if (menu.style.display === 'none') { rebuildMenu(); menu.style.display = 'block'; } else { menu.style.display = 'none'; } }
        function hideMenu(){ menu.style.display = 'none'; }

        document.addEventListener('click', hideMenu);
        try { yt.addEventListener('onPlaybackQualityChange', () => { /* optional refresh */ }); } catch(_){}

        const mount = () => { try { container.appendChild(wrap); } catch(_){} };
        const readyPoll = setInterval(() => { try { yt.getPlayerState(); clearInterval(readyPoll); mount(); } catch(_){} }, 150);

        // Rewire gear/settings button to open our Quality menu
        try {
            const controlsBar = player?.elements?.controls;
            const gearBtn = controlsBar?.querySelector?.('[data-plyr="settings"]') || null;
            const defaultMenu = player?.elements?.container?.querySelector?.('.plyr__menu') || null;
            if (defaultMenu) defaultMenu.style.display = 'none';
            if (gearBtn) gearBtn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); toggleMenu(); });
        } catch(_){}
    } catch (e) {}
}
