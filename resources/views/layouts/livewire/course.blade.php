...
<body class="bg-white min-h-screen" x-data="{ stage: '{{ $stage ?? 'pretest' }}', openSidebar: true, mobileSidebar: false }">
    ...
    <div class="flex mx-auto w-full h-[calc(100vh-4rem)] overflow-hidden">
        ...
        <x-course-layout.sidebar :stage="$stage ?? 'pretest'" :stages="$stages ?? ['pretest', 'module', 'posttest', 'result']" :progress="$progress ?? null" :modules="$modules ?? collect()"
            :active-module-id="$activeModuleId ?? null" :active-section-id="$activeSectionId ?? null" :completed-module-ids="$completedModuleIds ?? []" :close-route="route('courses.index')" :has-posttest-attempt="$hasPosttestAttempt ?? false"
            :can-retake-posttest="$canRetakePosttest ?? false" :can-retake-pretest="$canRetakePretest ?? false" :has-passed-posttest="$hasPassedPosttest ?? false" :course-id="$courseId ?? null" />
