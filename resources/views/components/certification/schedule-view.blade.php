<div x-data="{
                    ...
                    <div x-show="open" x-cloak @click.away="open=false"
                        ...
                        @foreach ($months as $idx => $label)
                            <button @click="$wire.setMonth({{ $idx + 1 }}); open=false"
                                ...
                                <span>{{ $label }}</span>
                                @php $cnt = $monthlySessionCounts[$idx+1] ?? 0; @endphp
                                @if ($cnt > 0)
    ...
    <div class="space-y-6">
        <div x-show="!mobile && activeView==='month'" x-cloak>
            <livewire:components.certification.month-grid :days="$days" :monthName="$this->monthName" :key="'cal-' . $currentYear . '-' . $currentMonth . '-' . $calendarVersion" lazy />
        </div>
        <div x-show="mobile || activeView==='agenda'" x-cloak>
            <livewire:components.certification.agenda-list :days="$days" :key="'agenda-' . $currentYear . '-' . $currentMonth . '-' . $calendarVersion" lazy />
        </div>
    </div>
</div>
