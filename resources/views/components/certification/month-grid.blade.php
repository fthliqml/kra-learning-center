                ...
                <div wire:click="addForDate('{{ $isoDate }}')"
                    ...
                    <div class="flex justify-between items-start p-1 sm:p-2">
                        ...
                        <button class="opacity-0 group-hover:opacity-100 transition text-gray-400 hover:text-primary"
                            title="Add certification" wire:click.stop="addForDate('{{ $isoDate }}')">
                    ...
                    @if (!empty($day['sessions']))
                        <div class="flex-1 overflow-y-auto px-1 flex flex-col gap-1">
                            @foreach ($day['sessions'] as $s)
                                ...
                                @php
                                    $start = $s['time']['start'] ?? null;
                                    $end = $s['time']['end'] ?? null;
                                    $isFaded = $s['is_faded'] ?? false;
                                    $color = $isFaded
                                        ? 'border-gray-300 bg-gray-50 hover:bg-gray-100'
                                        : 'border-yellow-500 bg-yellow-50 hover:bg-yellow-100';
                                    $opacity = $isFaded ? 'opacity-85' : '';
                                @endphp
                                <div wire:click.stop="open({{ $s['id'] }})" title="Open details"
                                    class="border-l-4 {{ $color }} {{ $opacity }} rounded-sm sm:rounded-md p-1 sm:py-1.5 sm:px-2 text-[10px] sm:text-xs flex flex-col gap-0.5 shadow-sm cursor-pointer">
                                    <div class="font-semibold leading-tight truncate flex items-center gap-1">
                                        @if ($type)
                                            @php
                                                $badgeColor = $isFaded
                                                    ? 'bg-white/70 border border-gray-300 text-gray-500'
                                                    : 'bg-white/70 border border-yellow-400 text-yellow-700';
                                            @endphp
                                            <span
                                                class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] sm:text-[10px] {{ $badgeColor }}">{{ $type }}</span>
                                        @endif
                                        <span class="truncate">{{ $s['title'] }}</span>
                                    </div>
                                    @if ($start && $end)
                                        <div class="text-[9px] sm:text-[11px] text-gray-600">
                                            {{ \Carbon\Carbon::parse($start)->format('H:i') }} -
                                            {{ \Carbon\Carbon::parse($end)->format('H:i') }}</div>
                                    @endif
                                    <div class="text-[9px] sm:text-[11px] text-gray-600 truncate">{{ $s['location'] }}
                                    </div>
                                </div>
                            @endforeach
                        </div>
