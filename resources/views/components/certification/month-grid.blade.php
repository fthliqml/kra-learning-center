                ...
                @if (!empty($day['sessions']))
                    <div class="flex-1 overflow-y-auto px-1 flex flex-col gap-1">
                        @foreach ($day['sessions'] as $s)
                            <div class="border-l-4 border-yellow-500 bg-yellow-50 rounded-sm sm:rounded-md p-1 sm:py-1.5 sm:px-2 text-[10px] sm:text-xs flex flex-col gap-0.5 shadow-sm cursor-pointer hover:bg-yellow-100"
                                wire:click="open({{ $s['id'] }})" title="Open details">
                                <div class="font-semibold leading-tight truncate flex items-center gap-1">
                                    @if (!empty($s['type']))
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] sm:text-[10px] bg-white/70 border border-yellow-400 text-yellow-700">
                                            {{ strtoupper($s['type']) }}
                                        </span>
                                    @endif
