<div class="space-y-4">
    @if ($readOnly && !in_array(auth()->user()?->role, ['section_head', 'dept_head', 'div_head', 'director']))
    ...
    <div class="flex justify-between items-start sm:items-center gap-3 sm:gap-4 w-full">
        ...
        @php $isLeader = in_array(auth()->user()?->role, ['section_head', 'dept_head', 'div_head', 'director']); @endphp
            ...
            <div class="max-h-[400px] overflow-y-auto thin-scrollbar">
                <table class="w-full text-sm border-collapse">
                    ...
                    <tbody>
                            ...
                            <tr class="even:bg-gray-50 hover:bg-gray-100 transition-colors">
                                ...
                                <td class="py-2.5 px-2 text-center">
                                    ...
                                    @else
                                        <x-select :options="[
                                            ...
                                            wire:model.defer="attendance.{{ $p['id'] }}.status"
                                            placeholder="Pending"
                                ...
                                <td class="py-2.5 px-2 text-center">
                                    ...
                                    @else
                                        <div class="flex justify-center">
                                            <input type="text"
                                                wire:model.defer="attendance.{{ $p['id'] }}.remark"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-8 text-center text-sm text-gray-500">No participants found
                                    for this certification.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div
            ...
            <div class="flex flex-wrap gap-3">
                <span class="inline-flex items-center gap-1"><span
                        class="w-2.5 h-2.5 rounded-full bg-green-500"></span>Present: {{ $present }}</span>
                <span class="inline-flex items-center gap-1"><span
                        class="w-2.5 h-2.5 rounded-full bg-red-500"></span>Absent: {{ $absent }}</span>
                <span class="inline-flex items-center gap-1"><span
                        class="w-2.5 h-2.5 rounded-full bg-amber-400"></span>Pending: {{ $pending }}</span>
            </div>
        </div>
    </div>
</div>
