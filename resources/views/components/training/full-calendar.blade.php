                        ...
                        @foreach ($day['trainings'] as $training)
                            ...
                            @php
                                ...
                                $typeColor = match ($type) {
                                    'IN' => 'border-green-500 bg-green-50 hover:bg-green-100',
                                    'OUT' => 'border-amber-500 bg-amber-50 hover:bg-amber-100',
                                    'K-LEARN',
                                    'KLEARN',
                                    'KLEARNING'
                                        => 'border-indigo-500 bg-indigo-50 hover:bg-indigo-100',
                                    default => 'border-primary bg-[#E4F3FF] hover:bg-[#d4ebfc]',
                                };
                                $typeLabel = $type ?: '';
                            @endphp
                            <div x-on:click="$dispatch('detail-loading-start')"
                                wire:click.stop="openTraining({{ $training['id'] }}, '{{ $isoDate }}')"
                                class="{{ $typeColor }} border-l-4 rounded-sm sm:rounded-md p-1 sm:py-1.5 sm:px-2 text-[10px] sm:text-xs flex flex-col gap-0.5 shadow-sm">
