<div x-data="{ calendarLoading: false }" 
     x-on:detail-loading-start.window="calendarLoading = true"
     x-on:training-modal-opened.window="calendarLoading = false"
     x-on:open-action-choice.window="calendarLoading = false"
     x-on:open-detail-training-modal.window="calendarLoading = false"
     class="relative">
    
    <!-- Loading Overlay -->
    <div x-show="calendarLoading" x-cloak
    ...
    <div class="bg-white rounded-lg overflow-hidden border border-gray-200 shadow">
        ...
        <div class="grid grid-cols-7">
            @foreach ($days as $day)
                ...
                <div 
                    @if($isAdmin) 
                        x-on:click="calendarLoading = true" 
                        wire:click="openAdd('{{ $isoDate }}')" 
                    @endif
                    ...
                    <div class="flex justify-between items-start p-1 sm:p-2">
                        ...
                        @if($isAdmin)
                            <button class="opacity-0 group-hover:opacity-100 transition text-gray-400 hover:text-primary"
                                title="Add training" 
                                x-on:click.stop="calendarLoading = true"
                                wire:click.stop="openAdd('{{ $isoDate }}')">
