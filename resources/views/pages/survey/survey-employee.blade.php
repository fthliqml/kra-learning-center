      ...
      <div
        ...
        <div class="p-4 md:p-5 flex flex-col h-full">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <h3 class="text-sm sm:text-base md:text-lg font-semibold text-base-content leading-snug truncate">
          ...
          <div class="mt-auto pt-3 flex items-center justify-between">
            ...
            @if ($survey->badge_status === 'complete')
              <a href="{{ route('survey.preview', ['level' => $survey->level, 'surveyId' => $survey->id]) }}">
                <x-button type="button" class="btn-xs sm:btn-sm border-primary/30 bg-primary/10" icon="o-eye"
                  label="Preview Survey" />
              </a>
            @elseif ($survey->start_disabled)
              <x-button type="button"
                class="btn-xs sm:btn-sm border-base-300 bg-base-200 text-base-content/40 cursor-not-allowed" icon="o-play"
                label="Start Survey" disabled />
            @else
