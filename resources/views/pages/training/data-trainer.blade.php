<div>
  ...
  {{-- Skeleton Loading --}}
  <x-skeletons.table :columns="5" :rows="10" targets="search,filter,save,deleteTrainer,file,openCreateModal" />
    ...
    <x-form wire:submit.prevent="save" no-separator>
      ...
      @elseif ($mode === 'create')
          ...
          ['value' => 'external', 'label' => 'External'],
            ...
            option-label="label" placeholder="Select trainer type..." class="focus-within:border-0"
            :error="$errors->first('formData.trainer_type')" single clearable />
          ...
          @if (($formData['trainer_type'] ?? 'internal') === 'internal')
            <x-choices label="Trainer Name*" wire:model.live="formData.user_id" :options="$trainersSearchable"
              ...
              placeholder="Search trainer name..." class="focus-within:border-0" min-chars=2 hint="Type at least 2 chars"
              searchable single clearable wire:change="checkDuplicateTrainer" />
          @else
            <x-input label="Trainer Name*" placeholder="Name of the trainer..." wire:model.live.debounce.500ms="formData.name"
      ...
      <x-input label="Institution{{ $mode === 'edit' || $mode === 'create' ? '*' : '' }}"
        placeholder="Institution name..." wire:model.defer="formData.institution" class="focus-within:border-0"
        :error="$errors->first('formData.institution')" :readonly="$mode === 'preview'" />
      ...
      <div class="mt-3 space-y-2">
        ...
        @else
          <div class="flex flex-col gap-3">
            {{-- Current Signature Preview with Delete Button --}}
            @if (!empty($formData['signature_path']))
              <div class="flex items-center gap-3 p-3 rounded-md bg-emerald-50 border border-emerald-200">
                <div class="flex items-center gap-2 text-xs text-emerald-800 flex-1">
                  <x-icon name="o-check-circle" class="size-4 text-emerald-500" />
                  <span class="font-medium">Current:</span>
                  <img src="{{ asset('storage/' . $formData['signature_path']) }}" alt="Current signature"
                    class="h-12 max-w-[200px] object-contain border border-emerald-200 bg-white px-2 py-1 rounded" />
                </div>
                <x-ui.button type="button" variant="danger"
                  wire:click="$dispatch('confirm', { title: 'Delete Signature', text: 'Are you sure you want to delete this signature?', action: 'deleteTrainerSignature' })"
                  wire:target="deleteTrainerSignature" wire:loading.attr="disabled"
                  class="!btn-sm bg-rose-600 hover:bg-rose-700 border-rose-600 text-white">
                  <span wire:loading.remove wire:target="deleteTrainerSignature" class="flex items-center gap-1">
                    <x-icon name="o-trash" class="size-3" />
                    Delete
                  </span>
                  <span wire:loading wire:target="deleteTrainerSignature" class="flex items-center gap-1">
      ...
      @if (!empty($duplicateWarning) && $mode === 'create')
        <div class="bg-amber-50 border border-amber-200 rounded-md p-3">
          <div class="flex items-start">
            <x-icon name="o-exclamation-triangle" class="w-5 h-5 text-amber-500 mr-2 mt-0.5 flex-shrink-0" />
            <div>
