      ...
      <div class="px-4 py-3 space-y-3">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center gap-3 text-xs w-full">
          ...
          <x-ui.button type="button" variant="secondary" wire:click="uploadSignature"
            ...
            <span wire:loading.remove wire:target="uploadSignature,signatureFile" class="flex items-center gap-2">
              <x-icon name="o-arrow-up-tray" class="size-3" />
              Upload Signature
            </span>
            <span wire:loading wire:target="uploadSignature,signatureFile" class="flex items-center gap-2">
              <x-icon name="o-arrow-path" class="size-3 animate-spin" />
              Processing...
            </span>
          </x-ui.button>
      ...
      <x-table :headers="$headers" :rows="$approvals" striped class="[&>tbody>tr>td]:py-2 [&>thead>tr>th]:!py-3"
        ...
        @if ($showGenerateCertificatesOverlay)
          ...
          <div class="flex justify-center">
            <input type="checkbox" class="checkbox checkbox-sm checkbox-primary" wire:model.defer="selectedApprovalIds"
              value="{{ $approval->id }}" @disabled(!$canSelect) />
          </div>
          @endscope
        @endif
        ...
        <div class="flex justify-center gap-1">
          ...
          @if ($showDeptHeadRowActions)
            <x-button icon="o-x-mark" class="btn-circle btn-ghost p-2 bg-rose-600 text-white hover:bg-rose-700" spinner
              wire:click="reject({{ $approval->id }})" />

            <x-button icon="o-check" class="btn-circle btn-ghost p-2 bg-emerald-600 text-white hover:bg-emerald-700" spinner
    ...
    @if ($showGenerateCertificatesOverlay)
      <div class="relative w-full" style="min-height:56px;">
        <div class="absolute left-0 bottom-0 flex items-center gap-2 z-10 p-2">
          {{-- Reject Selected --}}
          <x-ui.button variant="danger" type="button" wire:click="rejectSelected" wire:target="rejectSelected"
          ...
          {{-- Approve Selected --}}
          <x-ui.button variant="success" type="button" wire:click="approveSelected" wire:target="approveSelected"
            wire:loading.attr="disabled" class="bg-emerald-600 hover:bg-emerald-700 border-emerald-600 text-white">
  ...
  <x-modal wire:model="modal" title="Training Detail" separator box-class="max-w-5xl h-fit">
            ...
            <x-table :headers="$this->participantHeaders()" :rows="$this->participants" striped
              ...
              <div class="flex justify-center">
                ...
                @elseif ($hasCertificate && $certNumber)
                  <a href="{{ route('certificate.training.view', $participant->assessment_id) }}" target="_blank"
    ...
    <x-slot:actions>
      ...
      @if ($showLevel1Actions || $showLevel2Actions)
        <x-ui.button variant="danger" type="button" wire:click="reject" wire:target="reject" wire:loading.attr="disabled"
