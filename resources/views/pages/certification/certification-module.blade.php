    ...
    @else
        ...
        <x-modal wire:model="modal" :title="$mode === 'create' ? 'Add Certification Module' : ($mode === 'edit' ? 'Edit Certification Module' : 'Preview Certification Module')" separator box-class="max-w-3xl h-fit">
            ...
            <x-form wire:submit.prevent="save" no-separator>
                ...
                <x-input label="Competency" placeholder="E.g. Mechanical Engineering"
                    wire:model.defer="form.competency" class="focus-within:border-0" :error="$errors->first('form.competency')"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-input label="Duration (minutes)" type="number" min="1"
                        wire:model.defer="form.duration" class="focus-within:border-0" :error="$errors->first('form.duration')"
                        :readonly="$mode === 'preview'" placeholder="E.g. 60" />
                </div>
                ...
                <x-textarea label="Major Component"
                    placeholder="Describe major components, e.g. All inner & outer parts"
