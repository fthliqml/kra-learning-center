<div class="p-2 md:px-8 md:py-4 mx-auto max-w-5xl relative min-h-[50vh]"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {{-- Status Card --}}
        <section class="rounded-xl border border-{{ $statusColor }}-200 bg-{{ $statusColor }}-50 p-6 md:col-span-1 flex flex-col items-center justify-center text-center">
            ...
            @if ($topAttempt)
                @if ($essayPending)
                    ...
                    <div class="mt-1 text-xs text-{{ $statusColor }}-600/60">MC Score: <span class="font-semibold">{{ $mcPct }}%</span></div>
                @else
                    <div class="mt-2 text-sm text-{{ $statusColor }}-700/80">Final Score: <span class="font-bold text-lg">{{ $totalPct }}%</span></div>
                @endif
            @else
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-6 md:col-span-2">
            ...
            @if ($topUnderReview)
                <div class="h-48 flex flex-col items-center justify-center text-center text-gray-500 bg-amber-50/50 rounded-lg border border-dashed border-amber-200">
                    <x-icon name="o-clock" class="size-10 text-amber-400 mb-3" />
                    <p class="text-sm font-medium text-amber-700">Awaiting Grading</p>
                    <p class="text-xs text-amber-600/70 mt-1 max-w-xs">The comparison chart will be displayed after all answers are graded.</p>
                </div>
            @elseif (!$topAttempt)
                <div class="h-48 flex flex-col items-center justify-center text-center text-gray-500 bg-gray-50/50 rounded-lg border border-dashed border-gray-200">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 mb-4">
            ...
            <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">Attempt #{{ $currentAttemptNum }}</span>
        </div>
                ...
                <div class="flex items-center gap-6">
                    <div class="flex-1 space-y-3">
                        <div class="flex justify-between items-center text-sm">
                            ...
                            <span class="font-medium text-green-600">{{ $correct }} / {{ $mcTotal }}</span>
                        </div>
                        <div class="flex justify-between items-center text-sm">
                    ...
                    <div class="w-20 h-20 flex-shrink-0 relative">
                        <div x-data="{ chart: null }" x-init="(() => {
                            ...
                            <div class="absolute inset-0 flex items-center justify-center text-xs font-bold text-emerald-600">{{ $mcPct }}%</div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                @else
                    <div class="flex items-end justify-between">
                        <div>
                            ...
                            <div class="text-3xl font-bold text-gray-900">{{ $essayScore }} <span class="text-sm font-normal text-gray-400">/ {{ $maxManual }}</span></div>
                        </div>
                        <div class="bg-green-50 text-green-700 px-3 py-1.5 rounded-full text-xs font-medium border border-green-100">
                ...
                <ul class="divide-y divide-gray-100">
                    @foreach ($attempts as $it)
                        ...
                        <li class="relative {{ $isCurrent ? '' : 'group hover:bg-gray-50' }} transition-colors">
                            ...
                            @if (!$isCurrent)
                                <a href="{{ route('courses-result.index', ['course' => $course->id, 'attemptId' => $it['id']]) }}" wire:navigate class="flex items-center justify-between p-4 pl-5 cursor-pointer">
                            @else
                                <div class="flex items-center justify-between p-4 pl-5 bg-primary/5">
                            @endif
                            
                            <div class="flex items-center gap-4">
                                <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold {{ $isCurrent ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600' }}">
                                    {{ $it['number'] }}
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-sm font-medium {{ $isCurrent ? 'text-primary' : 'text-gray-900' }}">
                                        Attempt #{{ $it['number'] }}
                                        @if ($isCurrent)
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                    ...
                                    @if ($it['passed'] ?? false)
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">Passed</span>
                                    @else
                                ...
                                @if (!$isCurrent)
                                    <x-icon name="o-chevron-right" class="size-4 text-gray-400 group-hover:text-gray-600 transition" />
                                @endif
                            </div>
