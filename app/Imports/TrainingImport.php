    ...
    public function collection(Collection $rows)
    {
        ...
        try {
            foreach ($rows as $index => $row) {
                ...
                $courseTitle = trim((string) ($row['course_title_lms'] ?? ''));
                ...
                if ($type === 'LMS') {
                    if ($courseTitle === '') {
                        $rowErrors[] = "Row {$excelRow}: Course Title (LMS) required for LMS";
                    } else {
                    ...
                    // Non LMS specific fields may be blank
                } else { // IN / OUT
                    if ($trainingName === '') {
                ...
                foreach ($period as $dateObj) {
                    TrainingSession::create([
                        ...
                        'trainer_id' => $type === 'LMS' ? null : $trainerId,
                        'room_name' => $type === 'LMS' ? ($roomName ?: null) : $roomName,
                        'room_location' => $type === 'LMS' ? ($roomLocation ?: null) : $roomLocation,
                        'start_time' => $type === 'LMS' ? null : $startTime,
                        'end_time' => $type === 'LMS' ? null : $endTime,
                    ]);
                    $day++;
                }
