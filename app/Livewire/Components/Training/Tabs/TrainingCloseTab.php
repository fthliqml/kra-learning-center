...
class TrainingCloseTab extends Component
{
    ...
    /**
     * When practical grade is changed from the UI, update numeric score
     */
    public function updatedTempScores($value, $key): void
    {
        // $key example: "123.practical_grade" or "123.posttest_score"
        [$assessmentId, $field] = explode('.', (string) $key, 2) + [null, null];

        if ($field === 'practical_grade') {
            $numeric = $this->gradeToNumeric($value);
            if (!isset($this->tempScores[$assessmentId])) {
                $this->tempScores[$assessmentId] = [
                    'pretest_score' => null,
                    'posttest_score' => null,
            ...
            $this->tempScores[$assessmentId]['practical_score'] = $numeric;
            $this->tempScores[$assessmentId]['practical_grade'] = $value ?: null;
        }
    }
            ...
            if ($isLms) {
                ...
                if (!$hasPosttest || !$hasPractical) {
                    ...
                    if (!$attendancePassed) {
                        ...
                        $requiredPracticalGrade = $this->training->module->practical_passing_score ?? 'C';
                        $theoryPassed = (float) $posttestScore >= $theoryPassingScore;

                        $actualPracticalGrade = $this->getGradeFromScore((float) $practicalScore);
                        $practicalPassed = $this->gradeRank($actualPracticalGrade) >= $this->gradeRank($requiredPracticalGrade);
                    ...
                    if ($isLms) {
                        ...
                        if (is_numeric($posttest) && is_numeric($practical)) {
                            ...
                            if (!$attendancePassed) {
                                ...
                                $actualPracticalGrade = $this->getGradeFromScore((float) $practical);
                                $practicalPassed = $this->gradeRank($actualPracticalGrade) >= $this->gradeRank($requiredPracticalGrade);
