...
class TrainingCloseTab extends Component
{
    ...
    public function loadTempScores()
    {
        ...
        foreach ($assessments as $assessment) {
            ...
            $this->tempScores[$assessment->id] = [
                // Keep null when not attempted to avoid displaying misleading "0".
    ...
    protected function isCourseBasedTraining(): bool
    {
        $type = strtoupper((string) ($this->training?->type ?? ''));
        return in_array($type, ['LMS', 'BLENDED'], true);
    }
    ...
    /**
     * For course-based trainings (LMS, BLENDED), sync pretest and posttest scores from Course tests.
     * Uses STATUS_SUBMITTED attempts as "attempted"; under-review attempts are handled via getTestReviewStatus().
     */
    protected function syncLmsScoresFromPosttest(): void
    {
        ...
        foreach ($assessments as $assessment) {
            ...
            // LMS has no practical score; BLENDED may still use practical in closing flow.
            if ($trainingType === 'LMS') {
    ...
    protected function syncInternalTrainingScores(): void
    {
        ...
        $trainingType = strtoupper((string) ($this->training?->type ?? ''));
        // Some legacy records store internal trainings with null/empty type.
        if (!in_array($trainingType, ['IN', ''], true)) {
            return;
        }
        ...
        foreach ($assessments as $assessment) {
            ...
            $this->tempScores[$aid]['pretest_score'] = $this->tempScores[$aid]['pretest_score'] ?? null;
            $this->tempScores[$aid]['posttest_score'] = $this->tempScores[$aid]['posttest_score'] ?? null;
            ...
            if ($pretest) {
                ...
                if ($pretestAttempt) {
                    ...
                    $this->tempScores[$aid]['pretest_score'] = null;
                    $this->missingPretestCount++;
                }
            }
    ...
    /**
     * Get test review status for each employee.
    ...
    public function getTestReviewStatus(): array
    {
        ...
        if (in_array($trainingType, ['IN', ''], true)) {
            ...
            $pretest = Test::where('training_module_id', $moduleId)->where('type', 'pretest')->first();
            $posttest = Test::where('training_module_id', $moduleId)->where('type', 'posttest')->first();
        } elseif (in_array($trainingType, ['LMS', 'BLENDED'], true)) {
            $courseId = (int) ($this->training?->course_id ?? 0);
            if ($courseId <= 0) {
                return [];
            }

            $pretest = Test::where('course_id', $courseId)->where('type', 'pretest')->first();
            $posttest = Test::where('course_id', $courseId)->where('type', 'posttest')->first();
        } else {
            return [];
        }
