...
class TestConfig extends Component
{
    ...
    // Pretest settings
    // Pretest no passing score required (hidden in UI); keep property for backward compatibility
    public $pretest_passing_score = 0; // forced to 0 when saving
    public $pretest_max_attempts = 1; // default 1 (can be set to null for unlimited)
    ...
    protected function hydrateFromCourse(): void
    {
        ...
        if ($pre) {
            ...
            $this->pretest_passing_score = 0; // hidden/not applicable
            $this->pretest_max_attempts = $pre->max_attempts ?? 1; // default 1 if null
    ...
    public function saveDraft(): void
    {
        ...
        // Basic validation
        // Pretest passing score not used; normalize to 0
        $this->pretest_passing_score = 0;
        $this->posttest_passing_score = (int) $this->posttest_passing_score;
        // Remove pretest passing score validation — not applicable
        ...
        DB::transaction(function () {
            ...
            if (!$pre) {
                $pre = Test::create([
                    ...
                    'type' => 'pretest',
                    // Pretest passing score not applicable; persist as 0
                    'passing_score' => 0,
    ...
    {
        return view('components.skeletons.test-config');
    }
}
