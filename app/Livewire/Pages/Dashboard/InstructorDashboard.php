...
class InstructorDashboard extends Component
{
    ...
    // Survey 1 pending (participants who have not completed)
    public int $pendingSurvey1Count = 0;
    ...
    public function mount()
    {
        ...
        $this->loadPendingSurvey1Count();
    }

    public function loadPendingSurvey1Count(): void
    {
        $this->pendingSurvey1Count = 0;
        ...
        // Count pending Survey Level 1 responses for trainings where this user is a trainer.
        // "Pending" = survey is not draft, training is done/approved, and response is missing OR not completed.
        // Use whereExists for sessions to avoid row multiplication (multiple sessions per training).
        $this->pendingSurvey1Count = (int) DB::table('training_surveys as ts')
            ...
            ->whereExists(function ($q) use ($trainer) {
                $q->select(DB::raw(1))
                    ->from('training_sessions as s')
                    ->whereColumn('s.training_id', 't.id')
                    ->where('s.trainer_id', $trainer->id);
            })
            ->where(function ($q) {
                $q->whereNull('sr.id')
                    ->orWhere('sr.is_completed', 0);
            })
            ->count(DB::raw('DISTINCT CONCAT(ts.id, ":", a.employee_id)'));
    }
